...
class UserDataHandler:
    def __init__(self):
        self.con = psycopg2.connect(
            database="testdb", user="postgres", password="postgres", host=config.db_host, port="5432"
        )
    ...
    def add_user(self, data: dict):
        ...
        self.cursor.execute(
            "INSERT INTO public.users (email,password) VALUES ( '{}', '{}' );".format(data["email"], data["password"])
        )
        self.con.commit()
        return True
